...
shinyServer(function(input, output) {
  ...
  nuageDat <- reactive({
    input$alea
    res <- data.frame(cellule= sample(1:16, 160, replace = T), 
                      xReal= rnorm(160), 
                      yReal= rnorm(160), 
                      label= paste(1:160))
    res$xNorm <- (res$xReal - min(res$xReal)) / (max(res$xReal) - min(res$xReal))
    res$yNorm <- (res$yReal - min(res$yReal)) / (max(res$yReal) - min(res$yReal))
    res
  })
