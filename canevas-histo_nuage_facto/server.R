...
shinyServer(function(input, output) {
  ...
  output$thePlot <- reactive({ 
    sizeInfo <- input$taillegraph
    gridInfo <- list(nbLines=4,nbColumns=4,topology= input$topo)
    ...
    list(
      ...
      nuageData= le.nuage,
      nuageRange= list(range(do.call(c, sapply(le.nuage, function(tmp) tmp$x))), 
                       range(do.call(c, sapply(le.nuage, function(tmp) tmp$y)))),
  ...
  nuageDat <- reactive({
    input$alea
    res <- data.frame(x= rnorm(160, mean= 1), 
                      y= rnorm(160, mean= 1), 
                      label= paste(1:160))
    as.list(by(res, sample(1:16, 160, replace = T), identity))
  })
